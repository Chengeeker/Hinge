...
- 修复 QQ 隐藏到托盘后，Hinge 直接显示其内部 `Chrome_WidgetWin_1` 窗口，造成没有任务栏入口、界面无法交互的问题；
- 隐藏状态不再调用 `ShowWindow` 或 `ShowWindowAsync`，改为查询 QQ 自己的 Electron 托盘宿主和真实通知图标 ID，再投递与用户单击托盘图标等价的回调；
- 只有客户端已经自行显示的窗口才执行标准前台激活，避免外部程序绕过 QQ/微信自己的窗口生命周期；
- 更新 Electron 参考实现的合规记录；没有新增或打包 Electron 运行时依赖。
