...
`Weixin.exe`/`WeChat.exe`/`QQ.exe`/`QQNT.exe`/`QQEX.exe` 是否已经运行。已经由客户端显示的带标题顶层窗口只执行标准前台激活。QQ 或旧版 Electron 客户端隐藏到托盘时，Windows 端定位同进程的 `Electron_NotifyIconHostWindow`，使用 `Shell_NotifyIconGetRect` 查询真实通知图标 ID，并投递 Electron `WM_APP + 1` 回调。当当前 Qt 微信已在运行但没有可见主窗口时，Windows 端动态查找 `shell:AppsFolder` 中名为“微信”、`WeChat` 或 `Weixin` 的开始菜单登记项，通过系统应用激活入口唤起已有实例，不直接显示微信隐藏窗口，也不重新运行 `Weixin.exe`。Hinge 不显示隐藏的 Chromium/Qt 窗口、不绑定跨进程输入队列、不强制置顶，也不枚举或操作子窗口，
