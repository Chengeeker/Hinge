...
{
    [Theory]
    [InlineData("微信", "Microsoft.AutoGenerated.{1F1D4D0C}")]
    [InlineData("WeChat", "Microsoft.AutoGenerated.{ABCDEF}")]
    [InlineData("Weixin", "Tencent.Weixin")]
    public void DesktopChatAppRegistrationRules_AcceptsWeixinStartEntry(
        string displayName,
        string appId)
    {
        Assert.True(DesktopChatAppRegistrationRules.IsWeixinEntry(displayName, appId));
    }

    [Theory]
    [InlineData("微信输入法", "Microsoft.AutoGenerated.{INPUT}")]
    [InlineData("卸载微信", "uninstall.exe")]
    [InlineData("QQ", "Tencent.QQ")]
    [InlineData("微信", "")]
    public void DesktopChatAppRegistrationRules_RejectsUnrelatedStartEntries(
        string displayName,
        string appId)
    {
        Assert.False(DesktopChatAppRegistrationRules.IsWeixinEntry(displayName, appId));
    }
